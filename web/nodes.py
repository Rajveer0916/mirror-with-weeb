...
def make_tree(res):
    ...
    for i in res:
        ...
        if len(folders) > 1:
            ...
            TorNode(folders[-1], is_file=True, parent=previous_node, progress=i.progress, size=i.size, priority=i.priority, file_id=i.id)
        else:
            # at the file to the parent if no folders are there
            TorNode(folders[-1], is_file=True, parent=parent, progress=i.progress, size=i.size,priority=i.priority, file_id=i.id)
